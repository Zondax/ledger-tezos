...
import { ledger_fmt, MUTEZ_MULT, RPC_ADDR } from './common'
...
async function generate_vector(n: number): Promise<TestVector> {
  ...
  try {
    ...
    const randomBytesArr = Array.from({ length: 20 }, () => Math.floor(Math.random() * 255))
    ...
    const test_vector: TestVector = {
      name: `Simple Account Activation #${n}`,
      blob: forgedOp,
      operation: op,
      output: [
        { idx: 0, key: 'Operation', val: ledger_fmt(hash) }, //page 0
        { idx: 1, key: 'Type', val: ledger_fmt('Acct. Activation') }, //page 0
        { idx: 2, key: 'Public Key Hash', val: ledger_fmt(source) },
        { idx: 3, key: 'Secret', val: ledger_fmt(randomBytes) },
      ],
    }
