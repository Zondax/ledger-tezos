...
impl ApduHandler for BlindSign {
    ...
    fn handle<'apdu>(
        ...
        sys::zemu_log_stack("BlindSign::handle\x00");
...
impl Viewable for BlindSignUi {
    ...
    fn render_item(
        ...
        if let 0 = item_n {
            ...
            if m_len <= BlindSign::SIGN_HASH_SIZE * 2 {
                let chunk = self
                    .hash
                ...
                let n_pages = (BlindSign::SIGN_HASH_SIZE * 2) / m_len;
                Ok(1 + n_pages as u8)
            } else {
                hex::encode_to_slice(
                    &self.hash[..],
                    &mut message[..BlindSign::SIGN_HASH_SIZE * 2],
                )
                .map_err(|_| ViewError::Unknown)?;
                message[BlindSign::SIGN_HASH_SIZE * 2] = 0; //null terminate
                Ok(1)
            }
        } else {
...
mod tests {
    ...
    fn apdu_blind_sign() {
        ...
        let (_, out) = get_out().expect("UI mock not used");
